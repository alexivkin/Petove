...
help_and_leave() {
    echo "`basename $0` [-h] [-e] [-f] <name of the vm>"
    echo "  -h this help"
    echo "  -e the system is EFI"
    echo "  -f force to capture system information even if it's already present"
    exit 1
}
...
if [[ "$force" || ! -f dmi0.report ]]; then
    echo Capturing current hardware configuration...
    nvme=0
    PATH="$PATH:./" # use the local commands if the system does not provide them
    ...
    dmidecode --dump-bin dmi.bin >/dev/null
...
echo "  echo No disk clone \"$VMname.vdi\" found. Make sure to clone the source with dd and convert with \"VBoxManage convertfromraw\"."  >> $config_script
...
if [[ "$bios" == "efi" ]]; then
    echo "VBoxManage modifyvm \"$VMname\" --firmware efi" >> $config_script
fi
...
echo "VBoxManage storageattach \"$VMname\" --storagectl \"SATA Controller\" --device 0 --port 1 --type dvddrive --medium emptydrive" >> $config_script
...
# VBoxManage setextradata "$VMname" "VBoxInternal/Devices/$bios/0/Config/DmiSystemSerial"      "string:1234"

echo "# BIOS (dmi type 0)" >> $config_script
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBIOSVendor"        str "Vendor" dmi0.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBIOSVersion"       str "Version" dmi0.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBIOSReleaseDate"   str "Release Date" dmi0.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBIOSReleaseMajor"  rgx ".*BIOS Revision:\s\(.*\)\..*" dmi0.report # grep BIOS Revision (first number)
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBIOSReleaseMinor"  rgx ".*BIOS Revision:\s.*\.\(.*\)" dmi0.report # grep BIOS Revision (second number)
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBIOSFirmwareMajor" rgx ".*Firmware Revision:\s\(.*\)\..*" dmi0.report # may not be in dmi0
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBIOSFirmwareMinor" rgx ".*Firmware Revision:\s.*\.\(.*\)" dmi0.report # may not be in dmi0

echo "# System (dmi type 1)" >> $config_script
setParam "VBoxInternal/Devices/$bios/0/Config/DmiSystemVendor"    str "Manufacturer" dmi1.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiSystemProduct"   str "Product Name" dmi1.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiSystemVersion"   str "Version" dmi1.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiSystemSerial"    str "Serial Number" dmi1.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiSystemSKU"       str "SKU Number" dmi1.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiSystemFamily"    str "Family" dmi1.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiSystemUuid"      str "UUID" dmi1.report

echo "# Board (dmi type 2)" >> $config_script
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBoardVendor"       str "Manufacturer" dmi2.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBoardProduct"      str "Product Name" dmi2.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBoardVersion"      str "Version" dmi2.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBoardSerial"       str "Serial Number" dmi2.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBoardAssetTag"     str "Asset Tag" dmi2.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBoardLocInChass"   str "Location In Chassis" dmi2.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiBoardBoardType"    hex 14 dmi2.raw   # 14th byte in the header of the undecoded dmi in a decimal form" >> $config_script

echo "# Chassis (system enclosure)    (dmi type 3)" >> $config_script
setParam "VBoxInternal/Devices/$bios/0/Config/DmiChassisVendor"     str "Manufacturer" dmi3.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiChassisType"       hex 6 dmi3.raw # 6th byte in the header of chassis dmi, in decimal" >> $config_script
setParam "VBoxInternal/Devices/$bios/0/Config/DmiChassisVersion"    str "Version" dmi3.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiChassisSerial"     str "Serial Number" dmi3.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiChassisAssetTag"   str "Asset Tag" dmi3.report

echo "# Processor    (dmi type 4)" >> $config_script
setParam "VBoxInternal/Devices/$bios/0/Config/DmiProcManufacturer"  str "Manufacturer" dmi4.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiProcVersion"       str "Version" dmi4.report

echo "# OEM strings    (dmi type 11)" >> $config_script
setParam "VBoxInternal/Devices/$bios/0/Config/DmiOEMVBoxVer"        str "String 1" dmi11.report
setParam "VBoxInternal/Devices/$bios/0/Config/DmiOEMVBoxRev"        str "String 2" dmi11.report
...
#echo "VBoxManage  setextradata \"$VMname\" \"VBoxInternal/Devices/$bios/0/Config/BiosRom\" \"$vmscfgdir/pcbios.bin
#echo "VBoxManage  setextradata \"$VMname\"  \"VBoxInternal/Devices/$bios/0/Config/LanBootRom\" \"$vmscfgdir/pxerom.bin
